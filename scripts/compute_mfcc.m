function compute_mfcc( mainDir, prefix, wav_file )
...
close all
    ...
    frame_len = 400;
    frame_shift = 160;
    frame_num = ceil((length(s_in)-frame_len+frame_shift)/frame_shift);
    pad_len = ((frame_num-1)*frame_shift+frame_len)-length(s_in);
    t = (1:length(s_in)+pad_len)/f_s;
    
    %Remove DC offset and pad signal
    s_of = s_in-mean(s_in);
    s_of = [s_of;zeros(pad_len,1)];
    ...
    save(strcat('mfcc_data/mat/',prefix,'_feat_vect.mat'), 'feat');
